  ...
  template: `
    <div class="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-outline-light">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Departments Management</h1>
          <p class="text-sm text-gray-500 mt-1">Manage company departments.</p>
        </div>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <span class="text-gray-800 ml-1 md:ml-2">Departments</span>
                </div>
              </li>
            </ol>
          </nav>
        ...
        >
          <ng-template pTemplate="header">
            <tr class="bg-gray-50 border-b border-gray-100">
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider rounded-tl-2xl">Department</th>
            ...
            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end gap-2">
                  <button 
                    (click)="navigateToViewDepartment(department.id)"
            ...
            <tr>
              <td colspan="3" class="px-6 py-12 text-center text-gray-500">
                <div class="flex flex-col items-center justify-center">
                  ...
                  <p class="text-base font-medium text-gray-900">No departments found</p>
...
export class DepartmentMasterComponent implements OnInit {
    ...
    this.masterApi.getAllDepartments().subscribe({
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: error?.error?.message || error?.message || 'Failed to load departments',
        });
        this.isLoading.set(false);
      },
    });
  ...
  navigateToViewDepartment(departmentId: string): void {
    this.router.navigate(['/departments', departmentId, 'view']);
  }
  ...
  deleteDepartment(departmentId: string): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.masterApi.deleteDepartment(departmentId).subscribe({
          next: (response) => {
            if (response.success) {
              this.departments.set(this.departments().filter(r => r.id !== departmentId));
              this.messageService.add({
                severity: 'success',
                summary: 'Success',
          ...
          error: (error) => {
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: error?.error?.message || error?.message || 'Failed to delete department',
            });
          },
        });
      }
